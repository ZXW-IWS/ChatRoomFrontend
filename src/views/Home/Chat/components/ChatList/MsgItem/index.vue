...
import { formatTimestamp, getTimeCount } from '@/utils/computedTime'
import { useBadgeInfo, useUserInfo } from '@/hooks/useCached'
import type { CacheUserItem, MessageType, MsgType, ReplyType } from '@/services/types'
...
const scrollToMsg = async (msg: MsgType) => {
  ...
  if (index > -1) {
    virtualListRef?.value?.scrollToIndex(index, true, 12, timeCount)
  } else {
    // 如果没有加载过，就先加载，然后跳转
    ...
    setTimeout(
      virtualListRef?.value?.scrollToIndex(chatStore.getMsgIndex(reply.id), false, 12, timeCount),
      0,
    )
  }
  // 跳转到回复的消息 高亮一下
...
const replyNeedRender = (message: MsgType) => {
  const reply: ReplyType = message.body.reply
  return (
    reply.type === MsgEnum.IMAGE || reply.type === MsgEnum.VIDEO || reply.type === MsgEnum.EMOJI
  )
}
</script>
      ...
      <div :class="chatCls" v-if="!isRecall">
        ...
        <div class="chat-item-box" ref="boxRef">
          ...
          >
            ...
            <div
              ...
              :class="[
                ...
                { 'is-flash': chatStore.flashMsgId === props.msg?.message.id },
              ]"
          ...
          >
            ...
            <template class="ellipsis">
              {{ message.body.reply.username }}:
              <div
                v-if="replyNeedRender(message)"
                ref="renderMsgRef"
                :class="['chat-item-content', { uploading: message.body.reply?.loading }]"
                @contextmenu.prevent.stop="handleRightClick($event)"
              >
                <RenderMessage :message="message.body.reply" />
              </div>
              <span v-else>
