...
import { computedTimeBlock, setTimeCount } from '@/utils/computedTime'
...
export const useChatStore = defineStore('chat', () => {
  ...
  const getSessionList = async (isFresh = false) => {
    ...
    if (!isFirstInit) {
      ...
      getMsgList()
      // 请求第一个群成员列表
      // console.log("curRoom:",currentRoomType.value);
      // console.log("isGroup:",currentRoomType.value === RoomTypeEnum.Group)
  ...
  const sortAndUniqueSessionList = () => {
    ...
    sessionList.sort((pre, cur) => cur.hot_Flag - pre.hot_Flag)
  }
